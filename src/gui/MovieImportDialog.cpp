...
void MyMovieImportDialog::AddDirectoryClick( wxCommandEvent& event )
{
        ...
        //wxPrintf("event id:%d\n", event.GetId());
	wxString directory_path = event.GetString();
	...
	if (dlg.ShowModal() == wxID_OK)
	{
	    ...
	    wxDir::GetAllFiles 	( directory_path, &all_files, "*.mrc", wxDIR_FILES);
	    wxDir::GetAllFiles 	( directory_path, &all_files, "*.mrcs", wxDIR_FILES);
	    wxDir::GetAllFiles 	( directory_path, &all_files, "*.tif", wxDIR_FILES);
     }
    	all_files.Sort();
